...
需要认证。返回编号对应视频文件的字节流，供 `<video src="...">` 或播放器使用。
...
需要认证。仅当该编号对应视频为 **.ts** 时使用。返回按字节分片的 m3u8（`#EXT-X-BYTERANGE`），供前端 **hls.js** 等播放（浏览器多不原生支持 `video/mp2t`）。认证方式同视频流，支持 query 参数 `token`。
